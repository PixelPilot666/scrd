...
class DMD(nn.Module):
    def __init__(self, args):
        ...
        self.v_classifier = Classifier(input_size=512, hidden_size=512, num_classes=args.num_classes)
        self.t_classifier = Classifier(input_size=512, hidden_size=512, num_classes=args.num_classes)
        self.m_classifier = Classifier(input_size=512, hidden_size=512, num_classes=args.num_classes)
        self.classifier = Classifier(input_size=512, hidden_size=512, num_classes=args.num_classes)
    ...
    def forward(self, image, text):
        ...
        pre_m_in_t, _ = self.t_classifier(c_m)
        pre_v_in_m, _ = self.m_classifier(s_v)
        pre_t_in_m, _ = self.m_classifier(s_l)
        ...
        select_modal = self.modal_select_layer(torch.cat((feat_t,feat_v,feat_m),dim=1))
        ...
        s_l_att = feat_t.unsqueeze(1)
        s_v_att = feat_v.unsqueeze(1)
        c_m_att = feat_m.unsqueeze(1)
        ...
        # [:,modal_index,:]
        pre_m_att, feat_att = self.m_classifier(select_m)
        pre_msa ,_ = self.classifier(select_m)
