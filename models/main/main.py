...
class S2_VER:
    ...
    def train(self, args, epoch, best_eval_acc, logger=None):
        ...
        for (_, x_lb, t_lb, y_lb), (x_ulb_idx, x_ulb_w, x_ulb_s0, x_ulb_s1, \
                                    t_ulb_w, t_ulb_s0, t_ulb_s1, y_ulb) in tqdm(zip(self.loader_dict['train_lb'],
            ...
            t_ulb_w = [i.item() for i in t_ulb_w]
            # t_ulb_s0 = [i for i in t_ulb_s0]
            # t_ulb_s1 = [i for i in t_ulb_s1]
            # TODO text的输入格式
            # text_input = t_lb + t_ulb_w + t_ulb_s0 + t_ulb_s1
            text_input = t_lb + t_ulb_w + t_ulb_w + t_ulb_w
            ...
            with amp_cm():
                ...
                logits_x_lb = logits_msa[:num_lb]
                logits_t_lb = logits_t[:num_lb] 
                logits_v_lb = logits_v[:num_lb] 
                logits_m_lb = logits_m[:num_lb]
                # logits_x_ulb_w, logits_x_ulb_s = logits[num_lb:].chunk(2)
                logits_x_ulb_w, logits_x_ulb_s0, logits_x_ulb_s1 = torch.split(logits_msa[num_lb:], num_ulb)
                ...
                sup_loss = ce_loss(logits_x_lb, y_lb, reduction='mean')+ce_loss(logits_t_lb, y_lb, reduction='mean')+\
                            ce_loss(logits_v_lb, y_lb, reduction='mean')+ce_loss(logits_m_lb, y_lb, reduction='mean')
