...
mean['twitter'] = [0.485, 0.456, 0.406]
...
class Emotion_SSL_Dataset:
    ...
    def get_data(self, svhn_extra=True):
        ...
        if self.args.dataset == 'twitter':
            ...
            for id in anno.keys():
                img.append(np.array(Image.open(os.path.join(self.data_dir,data_path,id+'.jpg')).convert('RGB')))
                with open(os.path.join(self.data_dir,data_path,id+'.txt'),'r',encoding='ISO-8859-1') as file:
        ...
        if(self.train and self.isTum):
            anno_tum = json.load(open(os.path.join('/home/ubuntu/xwy/dataset/TumEmo','test_7.json')))
            for id in anno_tum.keys():
                # Image.open(os.path.join(self.data_dir,'data',id+'.jpg'))
                img.append(np.array(Image.open(os.path.join('/home/ubuntu/xwy/dataset/TumEmo/all_data',id+'.jpg')).convert('RGB')))
                with open(os.path.join('/home/ubuntu/xwy/dataset/TumEmo/all_data',id+'.txt'),'r',encoding='ISO-8859-1') as file:
                    text.append(file.read())
                    
                if(anno_tum[id]['label'] in [4,5]):
                    label.append(0)
                elif(anno_tum[id]['label'] in [0,1,3,6]):
                    label.append(1)
                else:
                    label.append(2)
                img_label.append(-1)
                text_label.append(-1)
        return img, text, img_label, text_label, label, init_len
        # for id in anno.keys():
        #     # Image.open(os.path.join(self.data_dir,'data',id+'.jpg'))
        #     img.append(np.array(Image.open(os.path.join(self.data_dir,'data',id+'.jpg')).convert('RGB')))
        #     with open(os.path.join(self.data_dir,'data',id+'.txt'),'r',encoding='ISO-8859-1') as file:
        ...
        #     label.append(anno[id]['label'])
        # init_len = len(img)
        # print('inintial dataset len: ' + str(len(img)))
        # if(self.train and self.isTum):
        #     anno_tum = json.load(open(os.path.join('/home/ubuntu/xwy/dataset/TumEmo','test_7.json')))
        #     for id in anno_tum.keys():
        #         # Image.open(os.path.join(self.data_dir,'data',id+'.jpg'))
    ...
    def get_dset(self, is_ulb=False,
        ...
        img, text, img_label, text_label, label, _ = self.get_data()
        num_classes = self.num_classes
        transform = self.transform
